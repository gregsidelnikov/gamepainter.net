...
//    print "Before database insert</br>";
    ...
    $PreviousUser = $Connection->get("user", "*", "`email_address` = '" . $RecipientEmail . "'", "", 1);

//    print_g($PreviousUser);

    if ($PreviousUser[0]['email_address'] == $RecipientEmail) {
        if ($PreviousUser[0]['account_status'] == 0)
            print "The user with this email address already exists, but not yet activated. Check your email for activation link.";
        else
            if ($PreviousUser[0]['account_status'] == 1)
